...
public class SmartObjectListRequester {
	...
	private static SmartObjectList parseList(String list) {
		...
		for (int i = 0; i < sos.size(); i++) {
			...
			if (serviceName == null || !services.get(5).equalsIgnoreCase(serviceName)) {
				if (soService != null)
					so.add(soService);
				soService = new SmartObjectService();
				// soService.setIdServiceModbus(services.get(4));
				soService.setServiceName(services.get(5));
				serviceName = services.get(5);
			}
			SOServiceParam param = new SOServiceParam();
			param.setIdRegisterModbus(services.get(4));
			param.setName(services.get(6));
			param.setType(ParamType.valueOf(services.get(7).toUpperCase()));
			param.setMinValue(services.get(8));
			param.setMaxValue(services.get(9));
			param.setOptions(services.get(10));
