...
public class SmartObject implements Serializable {
	...
	public SOServiceParam getParamByIDRegister(String idRegister) {
		SOServiceParam result = null;
		for (SmartObjectService service: this.services) {
			for(SOServiceParam i: service.getParams()) {
				if (i.getIdRegisterModbus().equals(idRegister)) {
					result = i;
					break;
				}
			}
